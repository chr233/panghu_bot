...
FONT_PATH = path.join(config.RES_PATH, 'sarasa-mono-sc-semibold.ttf')
OUTPUT_PATH = path.join(config.CACHE_PATH, 'monitor.png')


def gen_usage_graph() -> str:
    '''
    生成系统状态图
    ...
    try:
        ...
        path = draw_usage_data(system_data, usage_data)
...
def size2str(size: int) -> str:
    ...
    返回:
        形如 1.0KB 的格式
    '''
    def sos(integer: int, remainder: int, level: int) -> (int, int, int):
        if integer >= 1024:
...
def draw_usage_data(sysdata: list, usage: list) -> str:
    '''
    把系统数据绘成图片
