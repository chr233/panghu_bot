...
# @LastEditors  : Chr_
# @LastEditTime : 2020-09-02 18:09:54
...
async def add_steamid(conn: Connection, uid: int, steamid: int) -> bool:
    '''
    获取steam状态
    ...
    返回:
        bool: 操作是否成功
    '''
    sql = ("INSERT INTO `panghu`.`steam` (`uid`,`steamid`) SELECT %s,%s "
           "FROM DUAL WHERE NOT EXISTS (SELECT `qqid` FROM `panghu`.`users` "
           "WHERE `uid`='%s')")
    result = await exec_dql(conn, sql, (uid, steamid, uid))
    return(result)


async def get_steamid(conn: Connection, uid: int) -> (int, str, int, str, int, int, int, str):
    '''
    获取steam状态
    ...
    返回:
        int: steamid
    '''
    sql = ("SELECT `steamid` FROM `panghu`.`steam` WHERE `uid`=%s")
    result = await exec_dql(conn, sql, uid)
    return(result)


async def change_steamid(conn: Connection, uid: int, steamid: int) -> bool:
    '''
    修改steam数据
    ...
    返回:
        bool:
            操作是否成功
    '''
    sql = ("UPDATE `panghu`.`steam` SET `steamid`=%s WHERE `uid`=%s")
    result = await exec_dml(conn, sql,  (steamid,  uid))
    return(result)
